...
function renderList(){
  ...
  for(const d of filtered){
    ...
    row.innerHTML = `
      <div class="rowLeft">
        <img class="thumb" src="${d.photo || PLACEHOLDER_PHOTO}" alt="" />
        <div class="rowText">
          <div class="rowMeta">
...
async function saveDish(){
  ...
  const file = f_photo?.files?.[0] || null;
  ...
  if(file){
    try{
      ...
      uploadedPhotoPath = await githubUploadDishPhoto(file, name);

      saveBtn.textContent = oldText;
    }catch(e){
      console.error(e);
      alert("Не удалось загрузить фото в GitHub: " + e.message + "\nБудет использована заглушка/старое фото.");
  ...
  if(editMode === "add"){
    ...
    dishes = normalizeDishes([...dishes, {
      ...
      photo: uploadedPhotoPath || PLACEHOLDER_PHOTO,
      description
    }]);
    ...
    dishes = normalizeDishes(dishes.map(x=>{
      ...
      const photo = uploadedPhotoPath || x.photo || PLACEHOLDER_PHOTO;
